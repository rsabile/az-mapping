...
class TestSubscriptionInfo:
    """Tests for the /api/subscription-info endpoint."""

    def _load_fixture(self, name: str) -> dict:
        return json.loads((FIXTURES / name).read_text())

    def _mock_alias_success(self):
        """Return a side_effect that succeeds on the Alias API."""
        alias_data = self._load_fixture("subscription_alias.json")

        def _dispatch(url, **kwargs):
            resp = MagicMock()
            resp.raise_for_status.return_value = None
            resp.status_code = 200
            resp.json.return_value = alias_data
            return resp

        return _dispatch

    def test_alias_api_success(self, client):
        """Alias API returns createdDate → source='alias_api', isEstimated=False."""
        with patch(
            "az_mapping.services.subscription_age.requests.get",
            side_effect=self._mock_alias_success(),
        ):
            resp = client.get("/api/subscription-info?subscriptionId=sub-1")

        assert resp.status_code == 200
        data = resp.json()
        assert data["subscriptionId"] == "sub-1"
        assert data["createdDate"] == "2022-03-15T10:30:00.000000Z"
        assert data["source"] == "alias_api"
        assert data["isEstimated"] is False
        assert data["ageDays"] is not None
        assert data["ageDays"] > 0

    def test_alias_403_falls_back_to_rg(self, client):
        """Alias API returns 403 → falls back to oldest RG."""
        rg_data = self._load_fixture("resource_groups_list.json")

        def _dispatch(url, **kwargs):
            resp = MagicMock()
            resp.raise_for_status.return_value = None
            if "Microsoft.Subscription/subscriptions/" in url:
                resp.status_code = 403
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["source"] == "resource_group"
        assert data["isEstimated"] is True
        assert data["createdDate"] == "2023-01-10T08:00:00.000000Z"
        assert data["ageDays"] is not None
        assert data["ageDays"] > 0

    def test_both_fail_returns_unknown(self, client):
        """Both Alias and RG APIs fail → source='unknown'."""

        def _dispatch(url, **kwargs):
            resp = MagicMock()
            resp.raise_for_status.return_value = None
            if "Microsoft.Subscription/subscriptions/" in url or "/resourcegroups" in url:
                resp.status_code = 403
            else:
                resp.status_code = 200
                resp.json.return_value = {"value": []}
        ...
        assert data["ageDays"] is None

    def test_cache_hit(self, client):
        """Second call with same params hits cache — no second API call."""
        with patch(
            "az_mapping.services.subscription_age.requests.get",
            side_effect=self._mock_alias_success(),
        ) as mock_get:
            resp1 = client.get("/api/subscription-info?subscriptionId=sub-1")
            resp2 = client.get("/api/subscription-info?subscriptionId=sub-1")
    ...
    def test_missing_subscription_id(self, client):
        """Missing subscriptionId → 422 validation error."""
        resp = client.get("/api/subscription-info")
        assert resp.status_code == 422
